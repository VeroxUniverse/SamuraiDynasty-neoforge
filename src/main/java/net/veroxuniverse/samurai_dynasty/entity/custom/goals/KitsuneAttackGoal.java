...
public class KitsuneAttackGoal extends Goal {
    ...
    private final Runnable meleeAnimationCallback;
    private final Runnable rangedAnimationCallback;
    private Path path;
    ...
    public KitsuneAttackGoal(PathfinderMob mob,
                             double speedModifier,
                             boolean followingTargetEvenIfNotSeen,
                             Runnable meleeAnimationCallback,
                             Runnable rangedAnimationCallback) {
        this.mob = mob;
        this.speedModifier = speedModifier;
        this.followingTargetEvenIfNotSeen = followingTargetEvenIfNotSeen;
        this.meleeAnimationCallback = meleeAnimationCallback;
        this.rangedAnimationCallback = rangedAnimationCallback;
    ...
    public void tick() {
        ...
        if (livingentity != null && this.mob.distanceToSqr(livingentity) < 8.0D) {
            ...
            if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(livingentity))
                ...
                double dPath = this.mob.distanceToSqr(livingentity);
                if (dPath > 1024.0D) {
                    this.ticksUntilNextPathRecalculation += 10;
                } else if (dPath > 256.0D) {
            ...
            this.checkAndPerformAttack(livingentity, d0);
                ...
                && this.mob.distanceToSqr(livingentity) >= 8.0D) {
            ...
            if (this.mob.distanceTo(livingentity) >= 8.0D) {
                if (this.attackTime <= 0) {
                    ...
                    if (rangedAnimationCallback != null) {
                        rangedAnimationCallback.run();
                    }
    ...
    protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
        ...
        if (distToEnemySqr <= d0 && this.ticksUntilNextAttack <= 0) {
            ...
            if (meleeAnimationCallback != null) {
                meleeAnimationCallback.run();
            }
